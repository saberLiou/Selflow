        ...
        <tbody>
            @if (count($posts) > 0)
                @foreach ($posts as $post)
                    <tr>
                        <td>{{ $post->id }}</td>
                        <td><img height="50" width="50" src="{{ $post->photo ? Cloudder::secureShow($post->photo->post_directory.$post->photo->file) : 'https://placehold.it/50x50/?text=Unknown' }}" alt="{{ $post->photo ? $post->photo->file : 'Unknown' }}"></td>
